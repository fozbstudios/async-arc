...
{
	GCDAsyncSocket *asyncSocket = (GCDAsyncSocket *)CFBridgingRelease(connection);
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		if (value)
		{
			CFArrayRef certs = (CFArrayRef)CFBridgingRetain(value);
...
{
	GCDAsyncSocket *asyncSocket = CFBridgingRelease(CFBridgingRetain((__bridge GCDAsyncSocket *)pInfo));
